...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, h2_cache = affine_relu_forward(h1_out, self.params['W2'], self.params['b2'])
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        reg_1 = np.sum(self.params['W1'] * self.params['W1']) 
        reg_2 =  np.sum(self.params['W2'] * self.params['W2'])
        loss += 0.5 * self.reg * (reg_1 + reg_2)

        dx2, dw2, db2 = affine_relu_backward(dscore, h2_cache)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, d in enumerate(hidden_dims) :
          #print('W%d'%(i+1))
          self.params['W%d'%(i+1)] = weight_scale * np.random.randn(prior_d, d)
          self.params['b%d'%(i+1)] = np.zeros(d)
          prior_d = d
          if self.normalization == "batchnorm":
            self.params['gamma%d'%(i+1)] = np.ones(d)
            self.params['beta%d'%(i+1)] = np.zeros(d)

        self.params['W%d'%(self.num_layers)] = weight_scale * np.random.randn(prior_d, num_classes)
